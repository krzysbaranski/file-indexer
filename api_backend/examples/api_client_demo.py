...
def main():
    ...
    try:
        ...
        for file in python_files['files']:
            print(f"  - {file['path']}/{file['filename']} ({format_file_size(file['file_size'])})")
        ...
        print(f"Found {no_checksum_files['total_count']} files without checksums (showing first 5):")
        ...
        print(f"Found {pdf_files['total_count']} PDF files between 100KB and 10MB (showing first 3):")
        ...
        print(f"Found {duplicates['total_groups']} duplicate groups with {duplicates['total_duplicate_files']} total duplicate files")
        ...
        for dist in viz_data['size_distribution']:
            print(f"  {dist['size_range']}: {dist['count']:,} files ({format_file_size(dist['total_size'])})")
        
        print(f"\nTop file extensions:")
        for ext in viz_data['extension_stats'][:5]:
            print(f"  {ext['extension']}: {ext['count']:,} files ({format_file_size(ext['total_size'])})")
