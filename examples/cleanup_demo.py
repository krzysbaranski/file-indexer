    ...
    try:
        ...
        try:
            ...
            if individual_file.exists():
                print(
                    f"   Deleting individual file: {individual_file.relative_to(test_dir)}"
                )
            ...
            print(
                f"  - Deleted via directory check: {cleanup_result['files_deleted_by_directory']:,}"
            )
            print(
                f"  - Deleted via individual check: {cleanup_result['files_deleted_individually']:,}"
            )
            print(f"Deleted directories: {cleanup_result['deleted_directories']:,}")

            if cleanup_result.get("filesystem_calls_saved", 0) > 0:
                print(
                    f"Filesystem calls saved: {cleanup_result['filesystem_calls_saved']:,}"
                )
            ...
            print(
                f"  - Would check each file individually: {initial_stats['total_files']} checks"
            )
            print("\nWith optimization (new approach):")
            print(
                f"  - Directory checks: {cleanup_result['deleted_directories']} (found {cleanup_result['files_deleted_by_directory']} deleted files)"
            )
            print(
                f"  - Individual file checks: {cleanup_result['files_deleted_individually']} (only for files in existing directories)"
            )
            print(
                f"  - Total filesystem operations saved: {cleanup_result.get('filesystem_calls_saved', 0)}"
            )

            if cleanup_result.get("filesystem_calls_saved", 0) > 0:
                efficiency = (
                    cleanup_result["filesystem_calls_saved"]
